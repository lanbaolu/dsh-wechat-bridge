...
- **视频收发（iLink/ClawBot 协议）**：入站视频 CDN 下载+解密存 `.mp4` 交 agent；出站按扩展名路由 `VIDEO`（mp4/mov/webm/mkv/avi），构造 `video_item` 发送。**真机收发抽验通过（2026-08-30）**。语音仍为转写文本优先，语音出站无协议参考暂未实现（README 如实标注）。
- **待补发队列状态可视化**：面板新增「📮 待补发队列」，展示发送失败待补发数量/字数/最早入队时间。

### Changed

- **面板折叠分组重构**：拆为 5 个可折叠分组（⚙️ 状态与操作 / 🔗 连接与账号 / 💬 消息与通知 / ⚡ 系统行为 / 🔐 多用户与安全），默认只展开「状态与操作」，扫码绑定与操作按钮置顶——降低设置页复杂度。
